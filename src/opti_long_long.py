...
theta_init = np.arctan2((goal[1] - init[1]), (goal[0] - init[0]))
...
ax.set_ylim(0, 10)
ax.set_aspect('equal')
a1 = [(2.5, 2.5), (3.5, 2.5), (3.5, 10), (2.5, 10), (2.5, 2.5)]
a2 = [(7, 0), (8, 0), (8, 7.5), (7, 7.5), (7, 0)]
a3 = [(12, 2.5), (13, 2.5), (13, 10), (12, 10), (12, 2.5)]
a4 = [(16.5, 0), (17.5, 0), (17.5, 7.5), (16.5, 7.5), (16.5, 0)]

a5 = [(0, -1), (20, -1), (20, 0), (0, 0), (0, -1)]
a6 = [(0, 10), (20, 10), (20, 11), (0, 11), (0, 10)]
a7 = [(-1, 0), (0, 0), (0, 10), (-1, 10), (-1, 0)]
a8 = [(20, 0), (21, 0), (21, 10), (20, 10), (20, 0)]
gen_obs_ = [a1, a2, a3, a4, a5, a6, a7, a8]
OBS = ka.gen_obs(gen_obs_, ax)
plt.scatter(init[0], init[1], marker='o', color='lime', s=100)
plt.scatter(goal[0], goal[1], marker='o', color='red', s=100)


theta_init_goal = np.arctan2(goal[1] - init[1], goal[0] - init[0])
Bi_tree = bt.BinaryTree()
Order_dict = bt.OrderedKeyValueStructure()
ka_ = ka.kappa_init(init, goal, theta=theta_init, ka_limit=1.5,ka_dot_limit=20)
kappa_se = ka_
...
if obs_idx_first is not None:
    ...
    path_lib = path_first[:re_back_step, :2]
else:
    path = path_first
    path_lib = path_first[:re_back_step, :2]
while obs_idx_first is not None:
    ...
    if obs_idx_near_ccw is not None:
        ...
        path_lib = np.concatenate((path_lib, path_tmp_ccw[:obs_idx_near_ccw, :2]))
        tree_node.ori = 'l'
        ...
        path_lib = np.concatenate((path_lib, path_tmp_cw[:obs_idx_near_cw, :2]))
        tree_node.ori = 'r'
    ...
    ka_end = ka.kappa_init(re_back_point, goal, path_check_end[2], ka_limit=1.5,
                           ka_dot_limit=20)
    ...
    if last_collision_check_idx is None:
        ...
        if tmp <= 0:  # TODO
            ...
            conti_line_path = ka.integer_kappa(path_check_end[0], path_check_end[1], theta_init=path_check_end[2],
                                               kappa_seq=conti_ka)
            ...
            path_lib = np.concatenate((path_lib, conti_line_path[:, :2]))
            ...
            Order_dict.add_item(weight, path_seq_copy.copy())
            try_path_end_ka = ka.kappa_init(np.array([conti_line_path[-1, 0], conti_line_path[-1, 1]]), goal, theta=conti_line_path[-1, 2], ka_limit=1.5, ka_dot_limit=20)
            ...
            path_lib = np.concatenate((path_lib, try_path_end[:, :2]))
            ...
            if try_path_end_idx is not None:
                if try_path_end_idx > re_back_len:
                    ...
                    path_lib = np.concatenate((path_lib, try_path_end[:re_back_len, :2]))
                    tt = None
            ...
            path_lib = np.concatenate((path_lib, path_tmp[:tmp, :2]))
...
plt.plot(path_end[:, 0], path_end[:, 1], color='dodgerblue')
path_lib = np.concatenate((path_lib, path_end[:, :2]))
plt.plot(path_lib.transpose()[0], path_lib.transpose()[1], color='limegreen')
plt.show()
